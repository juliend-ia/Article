...
var ATOKEN = 'a92be39b486c2c2736d20f3b6e7a7d64b519c564ad44e4d266fdebe5b6c03ff0';
...
function supa(method, path, body) {
  ...
  return fetch(SURL + '/rest/v1/' + path, opts).then(function(r) { if (!r.ok) return r.text().then(function(t) { throw new Error(t); }); return r.status === 204 ? null : r.json(); });
}
...
function checkAuth() { if (localStorage.getItem(SKEY2) === ATOKEN) { document.getElementById('loginOverlay').classList.add('hidden'); loadArticles(); } }
...
document.getElementById('loginBtn').addEventListener('click', async function() {
  ...
  if (h === ATOKEN) { localStorage.setItem(SKEY2, ATOKEN); document.getElementById('loginOverlay').classList.add('hidden'); loadArticles(); }
...
document.getElementById('si').addEventListener('input', function() { displayCount = 30; expandedNum = null; doSearch(); });
...
function renderList(q) {
  ...
  for (var i = 0; i < n; i++) {
    ...
    var snum = esc(a.num);
    h += '<div class="card' + exp + '" data-num="' + snum + '"><div class="ct"><div><div class="cn">' + hl(a.nom,q) + '</div><div class="cc">' + esc(a.categorie||'') + '</div></div><div class="cnum">' + hl(a.num,q) + '</div></div><div class="det"><div class="dp"><div class="dl">N SAP</div><div class="dv">' + snum + '</div></div><div class="dp"><div class="dl">Categorie</div><div class="dv">' + esc(a.categorie||'--') + '</div></div><div class="dp"><div class="dl">Emplacement</div><div class="dv">' + loc + '</div></div>' + minmax + trow + photoRow + '<div class="cbtns"><div class="bedit" data-num="' + snum + '">Modifier</div><div class="bdel" data-num="' + snum + '">Supprimer</div></div><div class="btn-panier" data-num="' + snum + '">Ajouter au panier</div></div></div>';
  }
  con.innerHTML = h;
...
async function exportBon(id) {
  try {
    ...
    var csv = '\ufeffNumero ordre' + sep + 'Date' + sep + 'N SAP' + sep + 'Designation' + sep + 'Emplacement' + sep + 'Quantite' + nl;
    var date = new Date(bon.date_creation).toLocaleDateString('fr-FR');
    for (var i = 0; i < arts.length; i++) { var a = arts[i]; csv += bon.numero_ordre + sep + date + sep + a.num + sep + a.nom + sep + (a.location||'') + sep + a.qty + nl; }
